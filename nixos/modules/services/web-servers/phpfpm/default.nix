    ...
    in
    {
      options = {
        ...
        listen = mkOption {
          ...
          default = "";
          description = lib.mdDoc ''
            Disabled option -- remove in next release.
          '';
        };
...
in {
  imports = [
    (mkRemovedOptionModule [ "services" "phpfpm" "extraConfig" ] "Use services.phpfpm.settings instead.")
  ...
  config = mkIf (cfg.pools != {}) {
    ...
    assertions =
      ...
      mapAttrsToList (pool: poolOpts: ''
        Using config.services.phpfpm.pools.${pool}.listen is deprecated and will become unsupported in a future relea
e. Please reference the read-only option config.services.phpfpm.pools.${pool}.socket to access the path of your socket.
